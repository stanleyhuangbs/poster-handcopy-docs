---
name: poster-handcopy-docs
description: 海报手抄报图文文档制作；当用户提到海报、学生手抄报、历史画报、作业展示板、彩绘板、线条临摹参考版、封面、正文配图、配图、知识图解、内容拆图、批量生图、文章配图、小红书或公众号配图，或说“生成一个 logo / 图标 / 小图标 / app icon”，或指定内置 49 套内容风格与 8 套 logo/图标风格时使用。将中文或英文主题、文章、段落、知识点、学生作业或产品概念转成海报、手抄报、图文文档、封面图、正文配图、logo/图标与批量生图方案。
---
...
本分支优先于“强制风格门禁”和“选择器首显快速路径”。用户提到手抄报、历史画报、学生海报、作业展示板、彩绘板或线条临摹参考版时：

1. 读取 `references/student_handcopy_poster.md` 并进入学生手抄报模式，不按普通文章封面处理。
2. 先检查学生年龄或年级、主题、必须内容、表达重点、纸张与方向、语言是否已知；线稿文字保留方式是必选项，必须由选择器下拉菜单或对话明确确认后才能生成。
3. 继续补齐缺失信息时，每轮只问一个会改变结果的问题；不得重复询问用户已经说明的内容。
4. 把学生手抄报视为“以文字和思路为核心的纸面演示”，类似一页可手绘的特殊幻灯片；先确定结构与阅读路径，再放文字，最后只添加承担解释或导航作用的简笔线图。
5. 根据年龄调整文字深度、结构层级、色彩与临摹难度；绘画部分必须能在约 20 分钟内临摹完成，语言支持中文版、英文版或中英分别生成。
6. 信息齐备后先输出任务理解卡，再通过 `scripts/build_handcopy_selector.py` 展示三套推荐结构，并在下拉菜单提供更多可临摹结构模板和线稿文字保留方式。
7. 收到有效的 `CC2IMAGE_HANDCOPY_SELECTION_V1` 后，生成构图一致的彩绘完成版与线条临摹参考版；不要再打开普通风格选择器。
8. 中文或英文单语任务输出 2 张；中英分别生成时输出 4 张，不把双语内容挤在同一张画面。
9. 线稿版按 `tracing_text_mode` 生成：`gray_all_text` 灰度保留全部文字，`title_only` 只保留主标题，`title_and_subtitles` 保留主标题和小标题，`blank_structure` 完全无文字；全部结构线和保留文字统一为浅灰色，禁止黑色和深灰色。没有收到 `tracing_text_mode` 时不得猜默认值，必须先让用户选择。
10. 直接生成图片仍只能调用 `image_gen`；线稿版必须把彩绘版作为编辑输入以保持构图一致。
11. 临摹难度是交付硬门槛：最多 1 个简单主图和 3 个简笔小图；禁止复杂透视、群像、细密纹理、写实光影及纯装饰性绘画。未通过时先简化重做，不得交付。

## 强制风格门禁（最高优先级）

除学生手抄报模式外，本节优先于本 Skill 的所有默认值、自动匹配规则和直接生图规则。
...
普通内容或图标任务未指定风格时，首轮目标只有一个：**尽快显示选择器**。学生手抄报必须先完成年龄与内容对齐，不适用本快速路径。必须使用已经预校验的确定性生成器，禁止每次临时编写和调试 HTML。
...
把中文或英文文章、选题、段落、知识点或学生作业转成一套可批量生成的视觉资产：封面图、正文配图、内容拆图、学生手抄报、logo/图标和批量生图清单。内置 49 套内容视觉风格和 8 套 logo/图标风格；普通内容未指定风格时先通过可视化选择器让用户确认，学生手抄报先对齐任务再选择推荐结构、更多手抄报模板和线稿文字保留方式。学生手抄报流程不得在未确认线稿文字保留方式时直接生成。

## 默认执行原则

1. 先判断用户要的是：学生手抄报、封面图、正文配图、内容拆图规划、批量生图、还是 prompts/JSON 清单。
...
8. 普通内容未指定有效风格时，必须使用 `Visualize:visualize` 生成一次选择器；学生手抄报按专用流程逐项对齐后使用专用选择器。
