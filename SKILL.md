...
2. 先检查学生年龄或年级、主题、必须内容、表达重点、纸张与方向、语言是否已知；线稿文字保留方式由选择器下拉菜单确认。
...
6. 信息齐备后先输出任务理解卡，再通过 `scripts/build_handcopy_selector.py` 展示三套推荐结构，并在下拉菜单提供更多可临摹结构模板。
7. 收到有效的 `CC2IMAGE_HANDCOPY_SELECTION_V1` 后，生成构图一致的彩绘完成版与线条临摹参考版；不要再打开普通风格选择器。
8. 中文或英文单语任务输出 2 张；中英分别生成时输出 4 张，不把双语内容挤在同一张画面。
9. 线稿版按 `tracing_text_mode` 生成：`gray_all_text` 灰度保留全部文字，`title_only` 只保留主标题，`title_and_subtitles` 保留主标题和小标题，`blank_structure` 完全无文字；全部结构线和保留文字统一为浅灰色，禁止黑色和深灰色。
...
把中文或英文文章、选题、段落、知识点或学生作业转成一套可批量生成的视觉资产：封面图、正文配图、内容拆图、学生手抄报、logo/图标和批量生图清单。内置 49 套内容视觉风格和 8 套 logo/图标风格；普通内容未指定风格时先通过可视化选择器让用户确认，学生手抄报先对齐任务再选择推荐结构、更多手抄报模板和线稿文字保留方式。
